<template>
    <div>
        ...
        <el-row>
            <el-row>
                <el-button @click="onClickImportNode">{{
                    $t('message.importJob')
                }}</el-button>
                <el-button @click="onClickRemoveNode">{{
                    $t('message.deleteJob')
                }}</el-button>
                <el-button @click="onClickAddFrom">{{
                    $t('message.newStartPoint')
                }}</el-button>
                <el-button @click="onClickAddTo">{{
                    $t('message.newEndPoint')
                }}</el-button>
                <el-button @click="onClickRemoveEdge">{{
                    $t('message.deleteEdge')
                }}</el-button>
            </el-row>
            <div>
...
export default {
    ...
    methods: {
        ...
        draw() {
            console.log(
                'draw by data: ' +
                    JSON.stringify(this.workflowInfo.peworkflowDAG)
            )
            //获取D3
            ...
            inner.selectAll('g.node').on('click', e => {
                for (
                    let i = 0;
                    i < this.workflowInfo.peworkflowDAG.nodes.length;
                    i++
                ) {
                ...
                case 2: {
                    ...
                    if (this.from === undefined) {
                        this.$message.warning(
                            this.$t('message.ntfAddStartPointFirst')
                        )
                        break
                    }
                    if (this.from === to) {

    ...
    <div>
        <el-row>
            ...
            <el-col :span="1" :offset="17">
                <el-button type="success" @click="fetchWfInstanceInfo">{{
                    $t('message.refresh')
                }}</el-button>
            </el-col>
        </el-row>
        ...
        <el-row>
            <el-col :span="24">
                {{ $t('message.status') }}：
                <span class="title">{{
                    this.common.translateWfInstanceStatus(
                        wfInstanceDetail.status
                    )
                }}</span>
            </el-col>
        </el-row>
        ...
        draw: function() {
            ...
            let nodes = this.wfInstanceDetail.peworkflowDAG.nodes.map(node => {
                ...
                switch (node.status) {
                    case 3:
                        color = '#3498DB'
                        statusStr = this.$t('message.running')
                        break
                    case 4:
                        color = '#EC7063'
                        statusStr = this.$t('message.failed')
                        break
                    case 5:
                        color = '#58D68D'
                        statusStr = this.$t('message.success')
                        break
                    case 10:
                        color = '#F1C40F'
                        statusStr = this.$t('message.stopped')
                        break
                    default:
                        color = '#CACFD2'
                        statusStr = this.$t('message.waitingUpstream')
                        break
                }

                let l =
                    this.$t('message.jobId') +
                    ': ' +
                    node.jobId +
                    '\n' +
                    this.$t('message.jobName') +
                    ': ' +
                    node.jobName +
                    '\n' +
                    ...
                    if (node.jobId == e) {
                        if (node.instanceId == undefined) {
                            this.$message.warning(
                                this.$t('message.ntfClickWaitingNode')
                            )
                        } else {
